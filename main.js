...
function layout(active) {
  ...
  cards.forEach((card, i) => {
    ...
    card.style.transform =
      `translate3d(${off * gapX}px, 0, ${-abs * depth}px) rotateY(${off * angle}deg) scale(${s})`;
...
dots.forEach((d, i) => d.addEventListener('click', () => { index = i; layout(index); }));
...
stage.addEventListener('pointermove', e => {
  ...
  stage.style.transform = `rotateY(${(e.clientX - startX) / 90}deg)`;
});
